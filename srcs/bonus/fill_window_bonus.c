...
/*   Updated: 2023/01/19 22:25:20 by ntardy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	if_forest(t_data *data, int x, int y)
{
	...
	if (data->map_content.map[y][x] == 'M')
		mlx_put_image_to_window(data->ptr.mlx,
			data->ptr.win, data->images.ennemy_1, x * 64, y * 64);
...
int	fill_window(t_data *data)
{
	...
	mlx_string_put(data->ptr.mlx, data->ptr.win, 10, 32,
		0xFEFEFF, data->player.str_moves);
	return (1);
}
